...
class DeepLabV3(nn.Sequential):
    ...
    def __init__(self, n_classes, n_blocks, atrous_rates, multi_grids, output_stride):
        ...
        self.add_sublayer("layer2", _ResLayer(n_blocks[0], ch[0], ch[2], s[0], d[0]))
        self.add_sublayer("layer3", _ResLayer(n_blocks[1], ch[2], ch[3], s[1], d[1]))
        self.add_sublayer("layer4", _ResLayer(n_blocks[2], ch[3], ch[4], s[2], d[2]))
        self.add_sublayer(
            "layer5", _ResLayer(n_blocks[3], ch[4], ch[5], s[3], d[3], multi_grids)
        )
        self.add_sublayer("aspp", _ASPP(ch[5], 256, atrous_rates))
        concat_ch = 256 * (len(atrous_rates) + 2)
        self.add_sublayer("fc1", _ConvBnReLU(concat_ch, 256, 1, 1, 0, 1))
        self.add_sublayer("fc2", nn.Conv2D(256, n_classes, kernel_size=1))
